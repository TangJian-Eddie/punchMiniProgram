      ...
      return {
        jump: (dateInfo) => {
          if (dateInfo === undefined) {
            ...
            Object.prototype.toString.call(dateInfo) !== '[object Object]'
          ) {
            return;
          }
          const updatedRenderData = calcJumpData({
        ...
        cancelSelectedDates(cancelDates = []) {
          const existCalendarData =
            getCalendarData('calendar', component) || {};
        ...
        setDateStyle: (toSetDates) => {
          ...
          const toSetDatesStr = toSetDates.map((item) =>
            dateUtil.toTimeStr(item)
          );
